...
EventSelector_CP::EventSelector_CP(TTree* ) : fNumberOfEvents(0), fNumberOfGoodEvents(0), fTotalDataSize(0)
{
  ...
  Sigma_tbarW  = 11.17;
  Sigma_WJets  = 37509.0;
  Sigma_DYtautau1020 = 3708.0; 
  Sigma_DYtautau20   = 1871.0; 
...
void EventSelector_CP::SlaveBegin(TTree*)
{
   ...
   finvMGENF = new TH1D("invm_GENF", "invm_GENF", 742, 15,1500);
	...
	if( index == -1 ) continue;
	  ...
	  fdxyBS->Fill(Muon2_dxyBS[index],WEIGHT*FEWZ_WEIGHT);
        ...
        //} else if (histogramThis == "invm") { 
          ...
          if (recoMass_corr >= 15 && recoMass_corr <= 1500) fPU_weights->Fill(PU_WEIGHT,WEIGHT);
...
//dynamic_cast<TPair*>(fInput->FindObject("PROOF_CurrentElement")))) {
   ...
   if (z != "DATA" && Nntuple->Integral() != 0 && useNtupleWeightFlag != "True") { 
