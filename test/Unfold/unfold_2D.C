...
void unfold_2D(TString RR){
        ...
        cout << "../Inputs/unfold/resMatrixProd"+resRR+"_tmp.root" << endl;
        ...
        TString yRR = RR;
        if (RR == "POWHEG") yRR = "_";
        TFile* _inData = new TFile("../Inputs/rawYield/DYspectrum_Rap_uncorr_2013"+yRR+"tmp.root");
        _inData->cd();
	  ...
	  std::vector<double> input_err;
          ...
          TFile* f = new TFile("../Inputs/rawYield/DYspectrum_Rap_uncorr_2013"+yRR+"tmp.root");
          f->cd(); 
          //main 132 bin histos
          ...
          if (DATA_DRIVEN_BKG)  {
           ...
           ttbar = (TH1F*)gDirectory->Get("httbar");
           //case of OU 
           if (USE_OVERFLOWS && !DO_ROO) {
             ...
             httbar = (TH1F*)gDirectory->Get("httbar");
           }
          } else {
	...
	_outFile = new TFile("./rootfiles/unfoldData_HLT_Double_2D_"+RR+".root","recreate");
        _outFile->cd();
...
TH2F* getMigMatrix(TH1F* htrue, TH2F* hden) {
    ...
    for( int i = 0; i < nbin+1; i++ ) {
        for( int j = 0; j < nbin+1; j++ ) {
          ...
          //if (i==j)  cout << "XXX " << i << " " << migMatrix->GetBinContent(i+1,j+1) << " " << htrue->GetBinContent(i+1) << endl;
        }
    }
